 ...
 */
@SpringBootTest @AutoConfigureMockMvc class TestAgentApiIntegrationTests { @Autowired MockMvc mvc; private String token;
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Test void operatorCanDesignTests()throws Exception{mvc.perform(post("/api/ai/testing/design").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"serviceCode\":\"ORDER\",\"changeRisk\":4,\"changedFiles\":30,\"criticalFlows\":6,\"currentCoveragePercent\":68,\"flakyRatePercent\":7,\"databaseChange\":true,\"productionIncident\":false}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.priority").value("P0"));}}
